...
for i in range(100000):
    ...
    #fake = generator(z)
    #rep_z = represent(fake.detach(), False)
    #rep_x = represent(x, False)
    ...
    #mle_loss = generate_loss(fake, x)
    gan_loss = adv_loss(discriminator(fake), ones)
    #loss_G = mle_loss + gan_loss
    #loss_G = generate_loss(rep_z,rep_x)
    ...
    if i % 100 == 0:
        print(i, loss_R.item(), loss_D.item(), gan_loss.item())
        #print(i, loss_G.item())
        generator.eval()
        sample_plot = generator(sample)
        save_image(sample_plot.data, "C://유민형//개인 연구//Constrastive learning//sample3//%d.png" % i, nrow=5, normalize=True)
        #plt.show()
        #plt.close()
        generator.train()
