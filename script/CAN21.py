...
optim1 = LARS(optim.SGD(param1, lr=0.1)) #optim1 = optim.Adam(param1, lr=0.0001)
optim2 = LARS(optim.SGD(param2, lr=0.1)) #optim2 = optim.Adam(param2, lr=0.0001)
...
for iteration in range(10000000):
    ...
    if (iteration+1) % log_interval == 0:
        ...
        print("[Real contrastive loss:%f]" % real_loss.item(), end=" ")
        print("[Latent contrastive loss:%f]" % latent_loss.item())
        
        torch.save({'model_state_dict': repnet1.state_dict()}, "/home/compu/ymh/contrastive/CAN21_repnet1.pth")# "C://유민형//개인 연구//Constrastive learning//CAN21_repnet1.pth")
        torch.save({'model_state_dict': repnet2.state_dict()}, "/home/compu/ymh/contrastive/CAN21_repnet2.pth")# "C://유민형//개인 연구//Constrastive learning//CAN21_repnet2.pth")

        x_sample = torch.from_numpy(data.sample(1000)).float().to(device)
        z_sample = torch.from_numpy(np.random.normal(0,1,(1000,100))).float().to(device)

        x_rep = repnet1(x_sample).detach().cpu().numpy()
        plt.scatter(x_rep[:,0], x_rep[:,1], s=10, c='orange')
        plt.xlim(-1.5,1.5)
        plt.ylim(-1.5,1.5)
        plt.savefig("/home/compu/ymh/contrastive/CAN21/iter_%d_xrep.png" % (iteration+1))
        plt.show()
        plt.close()
        

        z_rep = repnet2(z_sample).detach().cpu().numpy()
        plt.scatter(z_rep[:,0], z_rep[:,1], s=10, c='green')
        plt.xlim(-1.5,1.5)
        plt.ylim(-1.5,1.5)
        plt.savefig("/home/compu/ymh/contrastive/CAN21/iter_%d_zrep.png" % (iteration+1))
        plt.show()
        plt.close()
