...
class IndividualLoss(nn.Module):
    ...
    def _make_matrix(self, x, y):
        matrix1 = self.similarity(x.unsqueeze(1), y.unsqueeze(0))
        matrix2 = self.L1_distance(x.unsqueeze(1), y.unsqueeze(0))
        return matrix1, matrix2

    def forward(self, representation):

        similarity_matrix1, similarity_matrix2 = self._make_matrix(representation, representation)
        #print(similarity_matrix1)
        #print(similarity_matrix2)
        
        unique1 = similarity_matrix1[self.mask]
        unique1 /= self.temperature
        
        #unique2 = similarity_matrix2[self.mask]
        #unique2 /= self.temperature
        
        log = torch.log(torch.exp(unique1).sum()) #/torch.exp(unique2).sum() + 
        #print(torch.exp(unique2).mean())
        #log = 1/torch.exp(unique2).mean()
