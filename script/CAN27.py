...
class sample_distribution:
    ...
    def point_random_sample(self, num, plot=False):
        index = np.random.choice(self.length, num)
        minibatch = self.point[index]
...
for i in range(100000):
    ...
    real, index = real_dist.point_random_sample(128,True)
    real = torch.from_numpy(real).float().to(device)
    representation = represent(real)
    
    latent = np.random.normal(0,0.1, (128,2))
    ...
    if i % 1 == 0:
        ...
        for j in range(fake.shape[0]):
            plt.scatter(fake[j,0], fake[j,1], s=100, c=real_dist.colors[index[j]], marker='x')
        plt.xlim(-1.5,1.5)
        plt.ylim(-1.5,1.5)
