...
test_idx = 10
pre_train = True
batch_num = 60000//pre_batch_size if pre_train else 60000//batch_size
...
#aug_mnist = Augmented_MNIST("/home/compu/ymh/contrastive/", transform_pretrain)
aug_mnist = Augmented_MNIST("/DATA/ymh/contrastive", transform_pretrain)
pretrain_loader = DataLoader(aug_mnist, batch_size=pre_batch_size, shuffle=True, pin_memory=True)
...
if pre_train:
    ...
    for epoch in range(1000):
        ...
        for i in range(batch_num):            
            aug1, aug2 = pretrain_loader.__iter__().next()
            ...
            out3 = discriminator(aug1,True)
            out4 = discriminator(aug2,True)
            
            rep3 = encoder(out3)
            rep4 = encoder(out4)            
            
            d_loss =  contrastive_loss(rep3, rep4)
            d_loss.backward()
            optim_pre_E.step()
            optim_pre_D.step()
        ...
        Eg_running_loss /= batch_num
        Ed_running_loss /= batch_num
        ...
        if (epoch+1) % 10 == 0:
            ...
            #model_name_discriminator = "/home/compu/ymh/contrastive/test" + str(test_idx) + "discriminator.pth"
            model_name_discriminator = "/DATA/ymh/contrastive/test" + str(test_idx) + "discriminator.pth"
            torch.save({'model_state_dict': discriminator.state_dict()}, model_name_discriminator)
        ...
        if (epoch+1) % 1 == 0:
            ...
            #save_image(fake_sample.data, "/home/compu/ymh/contrastive/sample5/epoch_%d.png" % (epoch+1), nrow=5, normalize=True)
    ...
    checkpoint1 = torch.load("/home/compu/ymh/contrastive/test9generator.pth")
    new_state_dict1 = OrderedDict()
    for k, v in checkpoint1['model_state_dict'].items():
        name = k[7:] # remove `module.`
        new_state_dict1[name] = v
    generator.load_state_dict(new_state_dict1)
    #generator.load_state_dict(checkpoint1['model_state_dict'])
    generator.train()
    
    checkpoint2 = torch.load("/home/compu/ymh/contrastive/test9discriminator.pth")
    new_state_dict2 = OrderedDict()
    for k, v in checkpoint2['model_state_dict'].items():
        name = k[7:] # remove `module.`
        new_state_dict2[name] = v
    discriminator.load_state_dict(new_state_dict2)
    #discriminator.load_state_dict(checkpoint2['model_state_dict'])
    discriminator.train()
    ...
    save_image(fake_sample.data, "/home/compu/ymh/contrastive/sample5/epoch_0.png", nrow=5, normalize=True)
    ...
    for epoch in range(200):
        ...
        for i in range(batch_num):
            ...
            real, _ = train_loader.__iter__().next()
            real = real.float().to(device)
