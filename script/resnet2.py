...
criterion1 = ContrastiveSupervisionLoss(10, device) 
...
for epoch in range(300): # 원하는 만큼
    ...
    for i in range(batch_num):
        ...
        x = x.float().to(device)
        y = y.long().to(device)
        #pos_mask, neg_mask = criterion1._prepare_data_mask(y)
        output = model(x)
        loss = criterion2(output, y)
        #loss = criterion1(output, y, pos_mask, neg_mask) + criterion2(output, y)
        loss.backward()
        optimizer.step()
