...
represent_z = Represent().to(device)
represent_x = Represent2().to(device)
generator = Generator().to(device)

optim_G = optim.Adam(generator.parameters(), lr=0.0001)
optim_R_z = optim.Adam(represent_z.parameters(), lr=0.0001)
optim_R_x = optim.Adam(represent_x.parameters(), lr=0.0001)
...
for i in range(100000):
    ...
    rep1_x = represent_x(x1)
    rep2_x = represent_x(x2)
    
    loss_R_x = contrastive_loss(rep1_x, rep2_x)
    loss_R_x.backward()
    optim_R_x.step()
    ...
    rep1_z = represent_z(z1)
    rep2_z = represent_z(z2)
    
    loss_R_z = contrastive_loss(rep1_z, rep2_z)
    loss_R_z.backward()
    optim_R_z.step()
    ...
    rep_z = represent_z(z, False)
    rep_x = represent_x(x, False)

    index, unique = matcher.match(rep_z, rep_x)
    #index = matcher.match(generator(z).view(-1,1024), x.view(-1,1024))
    z = z[index].detach()
    ...
    #loss_G = generate_loss(rep_z,rep_x)
    #loss_G = contrastive_loss(rep_z, rep_x)
    loss_G.backward()
    optim_G.step()
    
    if i % 100 == 0:
        print(i, loss_R_x.item(), loss_R_x.item(), loss_G.item(), unique)
        generator.eval()
        sample_plot = generator(sample)
        save_image(sample_plot.data, "C://유민형//개인 연구//Constrastive learning//sample4//%d.png" % i, nrow=5, normalize=True)
        #plt.show()
        #plt.close()
        generator.train()
